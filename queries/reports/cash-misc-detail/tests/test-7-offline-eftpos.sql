...
-- OFFLINE EFTPOS DATA: Show aggregated by drawer with verification stats (using CountedAmount)
SELECT
    cd.PosId AS POS,
    pt.Pod,
    ...
    CASE
        ...
        ELSE SUM(CASE WHEN tt.TenderTypeId = 9 THEN cdt.CountedAmount ELSE 0 END) /
             SUM(CASE WHEN tt.TenderTypeId = 9 THEN cdt.TransactionCount ELSE 0 END)
    END AS Average_OfflineEftpos,

    -- Verification columns
    SUM(SUM(CASE WHEN tt.TenderTypeId = 9 THEN cdt.CountedAmount ELSE 0 END)) OVER() AS Total_OfflineEftpos_Amount,
    SUM(SUM(CASE WHEN tt.TenderTypeId = 9 THEN cdt.TransactionCount ELSE 0 END)) OVER() AS Total_OfflineEftpos_Count,
    MIN(SUM(CASE WHEN tt.TenderTypeId = 9 THEN cdt.CountedAmount ELSE 0 END)) OVER() AS Min_Amount,
    MAX(SUM(CASE WHEN tt.TenderTypeId = 9 THEN cdt.CountedAmount ELSE 0 END)) OVER() AS Max_Amount
