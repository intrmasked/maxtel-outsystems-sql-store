...
SELECT
    ...
    CASE
        WHEN SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.TransactionCount ELSE 0 END) = 0 THEN NULL
        ELSE SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.DrawerAmount ELSE 0 END) /
             SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.TransactionCount ELSE 0 END)
    END AS Average_OfflineEftpos,

    -- Verification columns
    SUM(SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.DrawerAmount ELSE 0 END)) OVER() AS Total_OfflineEftpos_Amount,
    SUM(SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.TransactionCount ELSE 0 END)) OVER() AS Total_OfflineEftpos_Count,
    MIN(SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.DrawerAmount ELSE 0 END)) OVER() AS Min_Amount,
    MAX(SUM(CASE WHEN cdt.TenderTypeId = 9 THEN cdt.DrawerAmount ELSE 0 END)) OVER() AS Max_Amount
